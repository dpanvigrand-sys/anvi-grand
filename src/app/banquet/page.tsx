...
export const metadata: Metadata = { title: "Banquet" };
export const dynamic = "force-dynamic";

export default async function BanquetPage() {
  const venues = await getVenues("banquet");
  const venue = venues[0];
  if (!venue) {
    return <div className="mx-auto max-w-3xl px-5 py-20 text-center text-[var(--ag-muted)]">Banquet details unavailable.</div>;
  }
  return (
    <div>
      <section className="relative h-[45vh] min-h-[280px] overflow-hidden">
        <Image
          ...
          sizes="100vw"
          unoptimized={venue.image.startsWith("/uploads/")}
        />
        <div className="absolute inset-0 bg-[linear-gradient(160deg,rgba(227,27,35,0.72),rgba(74,28,22,0.65))]" />
        <div className="absolute inset-x-0 bottom-0 mx-auto max-w-6xl px-5 pb-10 md:px-8">
          <p className="text-xs uppercase tracking-[0.2em] text-white/70">Banquet</p>
      ...
      <section className="mx-auto grid max-w-6xl gap-12 px-5 py-14 md:grid-cols-[1.1fr_0.9fr] md:px-8">
        <div>
          <h2 className="font-display text-3xl text-[var(--ag-ink)]">Host at Anvi Grand</h2>
        ...
        <div className="border border-[var(--ag-line)] bg-white/80 p-5 md:p-8">
          <h3 className="font-display text-2xl text-[var(--ag-ink)]">Request a date</h3>
