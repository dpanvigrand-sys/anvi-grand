...
export default async function InwardPage() {
  ...
  const rows = stockMovesToRows(ops.inward, []).filter(
    (r) => r.direction === "inward",
  );
      ...
      <div className="grid gap-10 lg:grid-cols-2">
        <div>
          ...
          <p className="mt-2 text-[var(--ag-muted)]">
            Record stock received from vendors — with amount, advance, and
            balance. Reports: daily / monthly + Excel / Print A4 / JPG.
          </p>
        ...
        <div className="space-y-3">
          ...
          {ops.inward.length === 0 ? (
            <p className="border border-[var(--ag-line)] bg-white px-4 py-8 text-[var(--ag-muted)]">
              No inward moves yet — add one on the left.
            </p>
          ) : (
            ops.inward.slice(0, 8).map((m) => (
              <div
                key={m.id}
