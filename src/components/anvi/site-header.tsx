...
export function SiteHeader() {
  ...
  return (
    <header
      className={cn(
        "z-40 w-full",
        onHero ? "absolute inset-x-0 top-0" : "sticky top-0 border-b border-[var(--ag-line)] bg-[var(--ag-cream-white)]/95 backdrop-blur-sm",
      )}
    >
      <div className="mx-auto flex w-full max-w-6xl items-center justify-between gap-4 px-5 py-5 md:px-8">
        <Link
          ...
          className={cn(
            ...
            onHero ? "text-white drop-shadow-sm" : "text-[var(--ag-red)]",
          )}
        >
          ANVI GRAND
        </Link>
        ...
        <nav className="hidden items-center gap-5 lg:flex">
          {links.map((l) => (
            <Link
              ...
              className={cn(
                "text-sm transition",
                onHero ? "text-white/85 hover:text-white" : "text-[var(--ag-chocolate)] hover:text-[var(--ag-red)]",
              )}
            >
              {l.label}
            </Link>
          ...
          <Link
            ...
            className={cn(
              ...
              onHero ? "text-white/60 hover:text-white" : "text-[var(--ag-muted)] hover:text-[var(--ag-chocolate)]",
            )}
          >
            Ops
          </Link>
