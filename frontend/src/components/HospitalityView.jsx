...
export default function HospitalityView({ currentUser = null }) {
  ...
  async function printBooking(row, purpose = 'Final Bill', format = row.print_format || 'A4') {
    ...
    const isRestaurantBill = row.booking_type === 'FOOD';
    const businessTitle = isRestaurantBill ? (profile.restaurant_name || 'IRAA dine') : (profile.hotel_name || 'ANVI GRAND');
    const contactPhone = isRestaurantBill ? (profile.restaurant_phone || profile.phone || '') : (profile.reception_phone || profile.phone || '');
    ...
    const css = isThermal
      ? 'body{font-family:Arial,sans-serif;width:72mm;margin:0;padding:8px;color:#111;font-size:12px}.brand{display:flex;align-items:center;gap:4px;font-weight:900;color:#0f7490;font-size:12px;margin-bottom:4px}.brand img{width:24mm;height:auto}.center{text-align:center}.print-head{border-bottom:1px dashed #111;padding-bottom:7px;margin-bottom:7px}.print-head h1{font-size:16px;margin:0;text-transform:uppercase}.print-head h2{font-size:13px;margin:4px 0 0}.line{border-top:1px dashed #111;margin:8px 0}.row{display:flex;justify-content:space-between;gap:8px}.muted{font-size:11px;color:#444}table{width:100%;border-collapse:collapse}td{padding:2px 0;vertical-align:top}.total{font-weight:700;font-size:13px}'
      : 'body{font-family:Arial,sans-serif;margin:24px;color:#1f2937;background:#f8fafc}.bill{max-width:840px;margin:0 auto;border:1px solid #e5e7eb;padding:28px 32px;background:#fff;position:relative;box-shadow:0 12px 28px rgba(31,41,55,.08)}.brand{position:absolute;left:24px;top:22px;display:flex;align-items:center;gap:6px;font-weight:900;color:#0f7490;font-size:12px}.brand img{width:76px;height:auto}.print-head{text-align:center;border-bottom:2px solid #14532d;padding:0 100px 16px;margin-bottom:18px}.print-head h1{margin:0;color:#7f1d1d;font-size:30px;text-transform:uppercase;letter-spacing:0}.print-head h2{margin:6px 0 8px;color:#14532d;font-size:18px}.contact-line{color:#4b5563;font-size:12px;line-height:1.45}.grid{display:grid;grid-template-columns:1fr 1fr;gap:9px 24px}.line{border-top:1px solid #e5e7eb;margin:16px 0}table{width:100%;border-collapse:collapse;margin-top:10px}th,td{border-bottom:1px solid #eef2f7;padding:9px;text-align:left}th{background:#fff7ed;color:#4c1111}.amount{text-align:right}.total{font-weight:700;background:#f0fdf4}.muted{color:#6b7280;font-size:12px}.center{text-align:center}@media print{body{margin:0;background:#fff}.bill{border:0;box-shadow:none}.print-head{padding-left:96px;padding-right:96px}}';
    ...
    const qrText = row.upi_qr_text || (String(row.payment_mode || '').toLowerCase().includes('upi') ? `upi://pay?pn=${encodeURIComponent(businessTitle)}&am=${Number(row.balance_amount || row.total_amount || 0)}` : '');
